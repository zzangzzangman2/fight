...
public sealed class BattleMapAuthoringWindow : EditorWindow
{
    ...
    private void OnGUI()
    {
        mapData = (BattleMapData)EditorGUILayout.ObjectField("BattleMapData", mapData, typeof(BattleMapData), false);
        ...
        if (mapData == null)
        {
            EditorGUILayout.HelpBox("BattleMapData asset을 선택하거나 New 16x12를 누르세요.", MessageType.Info);
            return;
        }
    ...
    private void DrawBrush()
    {
        ...
        brushBlocksProjectiles = EditorGUILayout.Toggle("Blocks Projectiles", brushBlocksProjectiles);
    ...
    {
        string path = EditorUtility.SaveFilePanelInProject("Create BattleMapData", "baekdu_snow_gate_data",
                                                           "asset", "Create BattleMapData asset");
    ...
    private void NormalizeCells()
    {
        ...
        Vector2Int size = mapData.size.x <= 0 || mapData.size.y <= 0 ? new Vector2Int(16, 12) : mapData.size;
    ...
    private BattleCellData GetOrCreateCell(Vector2Int cell)
    {
        ...
        BattleCellData created = new BattleCellData { cell = cell, displayName = $"Cell {cell.x},{cell.y}" };
        if (BattleMapRuntimeCatalog.TryGetCell(BattleTestMapVariant.BaekduSnowGate, cell, out BattleMapRuntimeCell runtime))
        {
    ...
    private static Color CellColor(BattleCellData cell)
    {
        ...
        if (cell.tags != null && cell.tags.Exists(tag => string.Equals(tag, "stairs", StringComparison.OrdinalIgnoreCase)))
        {
