        ...
        private void OnGUI()
        {
            ...
            foreach (BattleTestTile tile in controller.PreviewTiles)
            {
                ...
                if (
                    screen.z < 0f
                    || screen.x < -80f
                    || screen.x > Screen.width + 80f
                    || screen.y < -80f
                    || screen.y > Screen.height + 80f
                )
                {
                    continue;
                }
        ...
        private Color ColorFor(BattleTestTile tile)
        {
            switch (mode)
            {
                ...
                case BattleMapDebugOverlayMode.Elevation:
                    return Color.Lerp(
                        new Color(0.10f, 0.40f, 1f, 0.34f),
                ...
                case BattleMapDebugOverlayMode.Cover:
                    return tile.coverBonus > 0
                        ? new Color(
                            ...
                            0.85f,
                            0.36f,
                            Mathf.Clamp01(0.24f + tile.coverBonus * 0.12f)
                        )
                ...
                case BattleMapDebugOverlayMode.MoveCost:
                    return tile.moveCost >= 90
                        ...
                        : Color.Lerp(
                            new Color(0.12f, 0.55f, 1f, 0.32f),
