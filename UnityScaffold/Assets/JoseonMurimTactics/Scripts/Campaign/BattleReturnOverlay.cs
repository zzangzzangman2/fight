    ...
    public sealed class BattleReturnOverlay : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            ...
            if (!reflectionReady && searchTimer >= FallbackGrace)
            {
                ...
                if (GUI.Button(new Rect(fb.x + 16f * s, fb.y + 10f * s, bw, 36f * s), "승리로 종료", UiTheme.ButtonPrimary)) Finish(true);
                if (GUI.Button(new Rect(fb.x + 32f * s + bw, fb.y + 10f * s, bw, 36f * s), "패배로 종료", UiTheme.Button)) Finish(false);
            }
        }
        ...
        private void DrawResultPending(float x, float pw, float s)
        {
            ...
            GUI.Label(new Rect(panel.x, panel.y + 58f * s, panel.width, 24f * s),
                $"결과 정리 중… ({remain:0.0}s)", UiTheme.SmallMuted == null ? UiTheme.Small : new GUIStyle(UiTheme.SmallMuted) { alignment = TextAnchor.MiddleCenter });

            float bw = 220f * s;
            if (GUI.Button(new Rect(panel.center.x - bw * 0.5f, panel.yMax - 44f * s, bw, 36f * s), "결과 보기 ▶", UiTheme.ButtonPrimary))
            {
                Finish(won);
            }
        }
        ...
        private bool DetermineWon()
        {
            ...
            if (unitsField != null && unitsField.GetValue(controller) is IEnumerable list)
            {
                foreach (object o in list)
                {
                    ...
                    if (u.definition.faction == Faction.Ally) alliesAlive = true;
                    else if (u.definition.faction == Faction.Enemy) enemiesAlive = true;
                }
            }
