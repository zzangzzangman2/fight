...
public sealed class DialogueController
{
    ...
    public void Draw(float screenW, float screenH)
    {
        ...
        if (choices)
        {
            ...
            for (int i = 0; i < current.choices.Count; i++)
            {
                ...
                string prefix =
                    c.disposition.HasValue ? $"[{StoryEnumLabels.Label(c.disposition.Value)}] " : string.Empty;
        ...
        else
        {
            ...
            if (GUI.Button(new Rect(box.xMax - pad - bw, box.yMax - pad - 44f * s, bw, 44f * s), nextLabel,
    ...
    private void DrawHistory(float screenW, float y, float s)
    {
        ...
        GUI.Label(new Rect(panel.x + 12f * s, panel.y + 8f * s, panel.width - 24f * s, 22f * s), "대화 로그",
                  UiTheme.SmallMuted);
        ...
        {
            GUI.Label(new Rect(panel.x + 12f * s, lineY, panel.width - 24f * s, 20f * s), history[i],
                      UiTheme.SmallMuted);
            lineY += 22f * s;
        }
    }
