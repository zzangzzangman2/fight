        ...
        public DialogueScript ToRuntimeScript()
        {
            ...
            foreach (DialogueNodeAsset nodeAsset in nodes)
            {
                DialogueNode node = new DialogueNode(
                    nodeAsset.nodeId,
                    nodeAsset.speakerName,
                    nodeAsset.line,
                    nodeAsset.nextNodeId,
                    nodeAsset.speakerId);

                foreach (DialogueChoiceAsset choiceAsset in nodeAsset.choices)
                {
                    DialogueChoice choice = new DialogueChoice(choiceAsset.text, choiceAsset.disposition, choiceAsset.nextNodeId);
                    foreach (IdDelta delta in choiceAsset.approvalDeltas) choice.approvalChanges.Add(delta);
                    foreach (IdDelta delta in choiceAsset.reputationDeltas) choice.factionChanges.Add(delta);
                    foreach (string flag in choiceAsset.setFlags) choice.flagsAdded.Add(flag);
                    foreach (IdDelta modifier in choiceAsset.battleModifiers) choice.battleModifiers.Add(modifier);
