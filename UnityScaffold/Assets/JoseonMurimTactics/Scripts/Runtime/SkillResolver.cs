    ...
    public sealed class SkillResolver
    {
        ...
        public BattleForecastData GetSkillPreview(CombatantRuntime actor, SkillData skill, CombatantRuntime target)
        {
            ...
            if (skill.pushDistance > 0)
            {
                ...
                movement.TryProjectPush(target.currentCell, direction, skill.pushDistance, out preview.pushDestination, out preview.pushCausesFall, out preview.pushBlocked);
        ...
        private SkillData FindCounterSkillAtCell(CombatantRuntime defender, CombatantRuntime attacker, Vector2Int defenderCell)
        {
        ...
        private RollMode ResolveRollMode(CombatantRuntime actor, Vector2Int actorCell, SkillData skill, CombatantRuntime target, Vector2Int targetCell)
        {
            int score = 0;
        ...
        private int TerrainAttackBonus(CombatantRuntime actor, Vector2Int actorCell, CombatantRuntime target, Vector2Int targetCell, SkillData skill)
        {
            int bonus = 0;
        ...
        private void ResolvePush(CombatantRuntime actor, SkillData skill, CombatantRuntime target)
        {
            ...
            bool fell;
            bool blocked;
            bool moved = movement.TryProjectPush(target.currentCell, direction, skill.pushDistance, out destination, out fell, out blocked);
        ...
        private bool TryFindCounterSkill(CombatantRuntime defender, CombatantRuntime attacker, Vector2Int defenderCell, out SkillData counterSkill)
        {
            counterSkill = null;
            if (defender == null || attacker == null || defender.defeated || defender.surrendered || !defender.actions.CanSpend(ActionSlot.Reaction))
            {
                return false;
            }
            ...
            {
                if (!IsAttackSkill(skill) || !CanPayCounterCost(defender, skill) || !CanReachWithSkill(defender, defenderCell, skill, attacker, attacker.currentCell))
                {
                    continue;
                }
        ...
        private bool CanReachWithSkill(CombatantRuntime actor, Vector2Int actorCell, SkillData skill, CombatantRuntime target, Vector2Int targetCell)
        {
        ...
        private bool IsAttackSkill(SkillData skill)
        {
            return skill != null &&
                ...
                !skill.tags.Contains(SkillTag.Movement) &&
                !skill.tags.Contains(SkillTag.Stance) &&
                !skill.tags.Contains(SkillTag.Formation) &&
                !skill.tags.Contains(SkillTag.Social);
        }

        private void FillAttackForecast(BattleForecastData preview, CombatantRuntime actor, SkillData skill, CombatantRuntime target, bool counter)
        {
            FillAttackForecast(preview, actor, actor.currentCell, skill, target, target.currentCell, counter);
        }

        private void FillAttackForecast(BattleForecastData preview, CombatantRuntime actor, Vector2Int actorCell, SkillData skill, CombatantRuntime target, Vector2Int targetCell, bool counter)
        {
            int distance = movement.GridDistance(actorCell, targetCell);
            int statMod = actor.StatModifier(skill.stat);
