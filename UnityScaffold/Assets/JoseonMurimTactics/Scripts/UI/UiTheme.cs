...
namespace JoseonMurimTactics
{
    ...
    public static class UiTheme
    {
        ...
        public static void DrawPanel(Rect rect, bool soft = false)
        {
            ...
            // 그림자
            GUI.DrawTexture(new Rect(rect.x + 5f * s, rect.y + 7f * s, rect.width, rect.height), texShadow, ScaleMode.StretchToFill);
        ...
        public static void DrawSeal(Rect rect, string glyph, float tilt = -5f)
        {
            ...
            {
                fontSize = Mathf.RoundToInt(rect.height * 0.46f),
        ...
        public static void EnsureStyles()
        {
            ...
            Body = Label(Mathf.RoundToInt(20 * Scale), FontStyle.Normal, Ink, TextAnchor.UpperLeft); Body.wordWrap = true;
            BodyCenter = Label(Mathf.RoundToInt(20 * Scale), FontStyle.Normal, Ink, TextAnchor.MiddleCenter); BodyCenter.wordWrap = true;
            Small = Label(Mathf.RoundToInt(16 * Scale), FontStyle.Normal, InkSoft, TextAnchor.UpperLeft); Small.wordWrap = true;
            SmallMuted = Label(Mathf.RoundToInt(15 * Scale), FontStyle.Normal, new Color(0.44f, 0.40f, 0.35f, 1f), TextAnchor.MiddleLeft);
        ...
        private static Font LoadKoreanFont()
        {
            ...
            Font f = null;
            try { f = Font.CreateDynamicFontFromOSFont(candidates, 22); }
            catch { f = null; }
            if (f == null) { f = Font.CreateDynamicFontFromOSFont("Arial", 22); }
            return f;
        }

        private static GUIStyle Label(int size, FontStyle style, Color color, TextAnchor anchor)
        {
            GUIStyle s = new GUIStyle { font = font, fontSize = Mathf.Max(8, size), fontStyle = style, alignment = anchor, richText = true };
        ...
        private static GUIStyle Btn(Texture2D normal, Texture2D hover, Texture2D active, Color textColor)
        {
            ...
            s.normal.background = normal; s.normal.textColor = textColor;
            s.hover.background = hover; s.hover.textColor = textColor;
            s.active.background = active; s.active.textColor = textColor;
            s.focused.background = normal; s.focused.textColor = textColor;
            return s;
        }

        private static GUIStyle Box(Texture2D bg, Color textColor)
        {
            GUIStyle s = new GUIStyle { font = font, border = new RectOffset(3, 3, 3, 3), padding = new RectOffset(14, 14, 12, 12) };
        ...
        {
            Texture2D t = new Texture2D(1, 1, TextureFormat.RGBA32, false) { hideFlags = HideFlags.HideAndDontSave, wrapMode = TextureWrapMode.Clamp, filterMode = FilterMode.Bilinear };
            t.SetPixel(0, 0, color); t.Apply();
            return t;
        }

        private static Texture2D VerticalGradient(Color top, Color bottom, int h)
        {
            Texture2D t = new Texture2D(2, h, TextureFormat.RGBA32, false) { hideFlags = HideFlags.HideAndDontSave, wrapMode = TextureWrapMode.Clamp, filterMode = FilterMode.Bilinear };
            for (int y = 0; y < h; y++)
            {
        ...
        {
            Texture2D t = new Texture2D(size, size, TextureFormat.RGBA32, false) { hideFlags = HideFlags.HideAndDontSave, wrapMode = TextureWrapMode.Repeat, filterMode = FilterMode.Bilinear };
        ...
        {
            Texture2D t = new Texture2D(size, size, TextureFormat.RGBA32, false) { hideFlags = HideFlags.HideAndDontSave, wrapMode = TextureWrapMode.Clamp, filterMode = FilterMode.Bilinear };
        ...
        {
            Texture2D t = new Texture2D(w, h, TextureFormat.RGBA32, false) { hideFlags = HideFlags.HideAndDontSave, wrapMode = TextureWrapMode.Clamp, filterMode = FilterMode.Bilinear };
            ...
            for (int x = 0; x < w; x++)
            {
                ...
                {
                    Color c = clear;
                    if (y < frontY) c = new Color(0.176f, 0.224f, 0.255f, 0.50f);
                    else if (y < backY) c = new Color(0.286f, 0.345f, 0.376f, 0.34f);
                    // 위로 갈수록 더 옅게 (안개)
                    if (c.a > 0f)
                    {
        ...
        private static Texture2D SealTex(int size)
        {
            ...
            for (int y = 0; y < size; y++)
            {
                for (int x = 0; x < size; x++)
                {
                    ...
                    Color c;
                    if (!inside) c = new Color(0f, 0f, 0f, 0f);
                    else if (border) c = new Color(0.529f, 0.149f, 0.118f, 1f); // 짙은 테
                    else c = SealRed;
                    t.SetPixel(x, y, c);
                }
            }
        ...
        {
            Texture2D t = new Texture2D(size, size, TextureFormat.RGBA32, false) { hideFlags = HideFlags.HideAndDontSave, wrapMode = TextureWrapMode.Clamp, filterMode = FilterMode.Point };
            for (int y = 0; y < size; y++)
            {
                for (int x = 0; x < size; x++)
                {
