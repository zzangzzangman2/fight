...
set "BUILD_LOG=%TEMP%\joseon-murim-mapeditor-build.log"
set "BUILD_ONLY="
...
echo.
echo [1/2] Building latest Map Editor player...
...
if errorlevel 1 (
  echo Map Editor player build failed.
  echo Log: %BUILD_LOG%
  pause
  exit /b 1
)
...
if not exist "%PLAYER_EXE%" (
  echo Map Editor executable was not created:
  echo %PLAYER_EXE%
  echo Log: %BUILD_LOG%
  pause
  exit /b 1
...
if defined BUILD_ONLY (
  echo.
  echo Map Editor build complete:
  echo %PLAYER_EXE%
  endlocal
  exit /b 0
)
...
echo.
echo [2/2] Launching Map Editor...
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0tools\launch-battletest-window.ps1" -ExePath "%PLAYER_EXE%" -Width 1600 -Height 900 --map-editor %*
exit /b %ERRORLEVEL%
