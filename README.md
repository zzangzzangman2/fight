...
설정: 박성준 18세, 동료 5인(백련·도아린·진서율·신서아·한비연)은 15세. 나이 데이터는 추후 계속 조정 예정. 선물은 유대(승인도 게이지)를 올리며, 기본 시스템 UI 표기는 "유대/호감"으로 통일한다(후속 지시 §2 — "연애도"를 기본 게이지 표기로 쓰지 않음). 연애 분기는 성인 전용 이벤트에서만 내부적으로 사용.
...
- 수입 기준: 새 게임 시작 은냥 300, 자유시간 반복 의뢰 38~55은냥, 객잔 품팔이 +35, 순찰 +10 — 시작 직후 선물 1~2개 + 초급 장비 1개 구매 가능.
- 재료 드랍: 도적 소굴 철광석, 늑대 고개 질긴 천, 호랑이 바위골 옥 조각, 표범 절벽길 철광석(반복 의뢰 승리 보상).
